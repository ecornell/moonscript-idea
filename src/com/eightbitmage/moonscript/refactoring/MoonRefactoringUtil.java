...
package com.eightbitmage.moonscript.refactoring;
 ...
 * Time: 6:56 AM
 */
public class MoonRefactoringUtil {
  @Nullable
  public static PsiElement getEnclosingContainer(PsiElement place) {
    ...
    while (true) {
      ...
      if (parent instanceof MoonBlock) {
        return parent;
      }
  ...
  @Nullable
  public static <T extends PsiElement> T findElementInRange(final MoonPsiFileBase file,
                                                            int startOffset,
                                                            int endOffset,
                                                            final Class<T> klass) {
    ...
    if (MoonTokenTypes.WHITE_SPACES_SET.contains(element1.getNode().getElementType())) {
      startOffset = element1.getTextRange().getEndOffset();
      element1 = file.getViewProvider().findElementAt(startOffset, file.getLanguage());
    }
    if (MoonTokenTypes.WHITE_SPACES_SET.contains(element2.getNode().getElementType())) {
  ...
  public static PsiElement[] getExpressionOccurrences(@NotNull PsiElement expr, @NotNull PsiElement scope) {
    ...
    Comparator<PsiElement> comparator = new Comparator<PsiElement>() {
      public int compare(PsiElement element1, PsiElement element2) {
        ...
        if (element1 instanceof MoonParameter &&
            element2 instanceof MoonParameter) {
          final String name1 = ((MoonParameter) element1).getName();
          final String name2 = ((MoonParameter) element2).getName();
...
//    if (scope instanceof MoonConditionalLoop) {
//      PsiElement son = expr;
...
//    } else {
      collectOccurrences(expr, scope, occurrences, comparator,  scope instanceof MoonPsiFileBase);
//    }
...
//      if (goIntoInner || !(scope instanceof MoonPsiFileBase)) {
...
//    }
    ...
    public static boolean isAppropriateContainerForIntroduceVariable(PsiElement realContainer) {
        return realContainer instanceof MoonBlock;
    }
    ...
    public static boolean isIdentifier(String text) {
        ...
        lexer.start(text);
        if (lexer.getTokenType() != MoonTokenTypes.NAME) return false;
        lexer.advance();
