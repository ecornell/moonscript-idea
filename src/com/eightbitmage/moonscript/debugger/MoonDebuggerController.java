 ...
 * Time: 6:31 PM
 */
public class MoonDebuggerController {
    private static final Logger log = Logger.getInstance("Lua.MoonDebuggerController");
    ...
    Map<XBreakpoint, MoonPosition> myBreakpoints2Pos = new HashMap<XBreakpoint, MoonPosition>();
    Map<MoonPosition, XBreakpoint> myPos2Breakpoints = new HashMap<MoonPosition, XBreakpoint>();

    Project myProject = null;

    MoonDebuggerController(XDebugSession session) {
    ...
    public void addBreakPoint(XBreakpoint breakpoint) {
        try {
            MoonPosition pos = MoonPositionConverter.createRemotePosition(breakpoint.getSourcePosition());
    ...
    private void processResponse(String messages)  {
        ...
        for (int i = 0, linesLength = lines.length; i < linesLength; i++) {
            ...
            if (m.matches()) {
                ...
                MoonPosition position = new MoonPosition(file, Integer.parseInt(line));

                XBreakpoint bp = myPos2Breakpoints.get(position);

                ready = true;

                MoonSuspendContext ctx;

                if (bp != null)
                   ctx = new MoonSuspendContext(myProject, this, bp, stack);
                else
                   ctx = new MoonSuspendContext(myProject, this, MoonPositionConverter.createLocalPosition(position), stack);
...
//                });
    ...
    private void processOK(String message) {
        ...
        if (executionRequest != null && message.length() > 0) {
            ...
            MoonDebugValue value = new MoonDebugValue(type, message);
