...
public class MoonPsiUtils {
    ...
    private static boolean isValidContainer(PsiElement element) {
        return element instanceof MoonBlock || element instanceof MoonPsiFile;
    }
    ...
    @Nullable
    public static MoonPsiElement getCoveringPsiElement(@NotNull final PsiElement psiElement) {
        PsiElement current = psiElement;
        while (current != null) {
            if (current instanceof MoonPsiElement) {
                return (MoonPsiElement) current;
            }
    ...
    public static boolean isLValue(MoonPsiElement element) {
      if (element instanceof MoonReferenceElement)
        if (((MoonReferenceElement) element).getElement().getParent().getParent() instanceof MoonIdentifierList)
            return true;

      if (element instanceof MoonSymbol)
        if (element.getParent().getParent() instanceof MoonIdentifierList)
            return true;
  ...
  @NotNull
  public static MoonDocPsiElement[] toPsiElementArray(@NotNull Collection<? extends MoonDocPsiElement> collection) {
    if (collection.isEmpty()) return MoonDocPsiElement.EMPTY_ARRAY;
    return collection.toArray(new MoonDocPsiElement[collection.size()]);
  }
  @NotNull
  public static MoonPsiElement[] toPsiElementArray(@NotNull Collection<? extends MoonPsiElement> collection) {
    if (collection.isEmpty()) return MoonPsiElement.EMPTY_ARRAY;
    return collection.toArray(new MoonPsiElement[collection.size()]);
  }
  @NotNull
