...
import static com.eightbitmage.moonscript.lang.parser.MoonElementTypes.NEWLINE;
import static com.eightbitmage.moonscript.lang.parser.MoonElementTypes.SHORTCOMMENT;


public class MoonSpacingProcessor extends MoonElementVisitor {
  private static final ThreadLocal<MoonSpacingProcessor> mySharedProcessorAllocator = new ThreadLocal<MoonSpacingProcessor>();
  protected MyMoonSpacingVisitor myLuaElementVisitor;
  protected static final Logger LOG = Logger.getInstance("org.jetbrains.plugins.lua.formatter.processors.MoonSpacingProcessor");

  private MoonSpacingProcessor(MyMoonSpacingVisitor visitor) {
//    super(visitor);
    myLuaElementVisitor = visitor;
  }

  public static Spacing getSpacing(MoonFormattingBlock child1, MoonFormattingBlock child2, CodeStyleSettings settings) {
    return getSpacing(child2.getNode(), settings);
  }

  private static Spacing getSpacing(ASTNode node, CodeStyleSettings settings) {
    MoonSpacingProcessor spacingProcessor = mySharedProcessorAllocator.get();
    try {
      if (spacingProcessor == null) {
        spacingProcessor = new MoonSpacingProcessor(new MyMoonSpacingVisitor(node, settings));
        mySharedProcessorAllocator.set(spacingProcessor);
      } else {
        spacingProcessor.setVisitor(new MyMoonSpacingVisitor(node, settings));
      }
  ...
  public void setVisitor(MyMoonSpacingVisitor visitor) {
    myLuaElementVisitor = visitor;
  }
   ...
   */
  private static class MyMoonSpacingVisitor extends MoonElementVisitor {
    ...
    public MyMoonSpacingVisitor(ASTNode node, CodeStyleSettings settings) {
      mySettings = settings;
      init(node);
    }
...
//
//
//    public void visitWhileStatement(MoonWhileStatement statement) {
