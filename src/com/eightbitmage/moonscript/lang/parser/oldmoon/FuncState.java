 ...
 */
package com.eightbitmage.moonscript.lang.parser.oldmoon;
...
public class FuncState {
    ...
    private static final Logger log = Logger.getInstance("Moon.Parser.FuncState");
...
//	LTable h;  /* table to find (and reuse) elements in `k' */
	...
	int indexupvalue(String name, ExpDesc v) {
		...
		_assert (v.k == MoonParser.VLOCAL || v.k == MoonParser.VUPVAL);
	...
	int singlevaraux(String n, ExpDesc var, int base) {
		...
		if (v >= 0) {
			var.init(MoonParser.VLOCAL, v);
			if (base == 0)
				markupval(v); /* local will be used as an upval */
			return MoonParser.VLOCAL;
		} else { /* not found at current level; try upper one */
			if (prev == null) { /* no more levels? */
				/* default is global variable */
				var.init(MoonParser.VGLOBAL, NO_REG);
				return MoonParser.VGLOBAL;
			}
			if (prev.singlevaraux(n, var, 0) == MoonParser.VGLOBAL)
				return MoonParser.VGLOBAL;
			var.info = indexupvalue(n, var); /* else was LOCAL or UPVAL */
			var.k = MoonParser.VUPVAL; /* upvalue in this level */
			return MoonParser.VUPVAL;
		}
	}

	void enterblock (BlockCnt bl, boolean isbreakable) {
	  bl.breaklist = MoonParser.NO_JUMP;
...
//	}
	...
	void closelistfield(ConsControl cc) {
		if (cc.v.k == MoonParser.VVOID)
			return; /* there is no list item */
		this.exp2nextreg(cc.v);
		cc.v.k = MoonParser.VVOID;
	...
	boolean hasmultret(int k) {
		return ((k) == MoonParser.VCALL || (k) == MoonParser.VVARARG);
	}
	...
	void lastlistfield (ConsControl cc) {
		...
		if (hasmultret(cc.v.k)) {
		    ...
		    this.setlist(cc.t.info, cc.na, MOON_MULTRET);
	...
	int jump() {
		...
		this.jpc = MoonParser.NO_JUMP;
		int j = this.codeAsBx(OP_JMP, 0, MoonParser.NO_JUMP);
	...
	void fixjump(int pc, int dest) {
		...
		_assert (dest != MoonParser.NO_JUMP);
	...
	int getjump(int pc) {
        try {
            ...
            if (offset == MoonParser.NO_JUMP)
                /* end of list */
                return MoonParser.NO_JUMP;
            else
...
//            log.warn("Jump control will attempt out of bounds index");
	...
	boolean need_value(int list) {
		for (; list != MoonParser.NO_JUMP; list = this.getjump(list)) {
	...
	int concat(int l1, int l2) {
		if (l2 == MoonParser.NO_JUMP)
			return l1;
		if (l1 == MoonParser.NO_JUMP)
			l1 = l2;
		else {
			int list = l1;
			int next;
			while ((next = this.getjump(list)) != MoonParser.NO_JUMP)
				/* find last element */
				list = next;
	...
	void setreturns(ExpDesc e, int nresults) {
		if (e.k == MoonParser.VCALL) { /* expression is an open function call? */
			SETARG_C(this.getcodePtr(e), nresults + 1);
		} else if (e.k == MoonParser.VVARARG) {
	...
	void setoneret(ExpDesc e) {
		if (e.k == MoonParser.VCALL) { /* expression is an open function call? */
			e.k = MoonParser.VNONRELOC;
			e.info = GETARG_A(this.getcode(e));
		} else if (e.k == MoonParser.VVARARG) {
			SETARG_B(this.getcodePtr(e), 2);
			e.k = MoonParser.VRELOCABLE; /* can relocate its simple result */
		}
	}

	void dischargevars(ExpDesc e) {
		switch (e.k) {
		case MoonParser.VLOCAL: {
			e.k = MoonParser.VNONRELOC;
			break;
		}
		case MoonParser.VUPVAL: {
			e.info = this.codeABC(OP_GETUPVAL, 0, e.info, 0);
			e.k = MoonParser.VRELOCABLE;
			break;
		}
		case MoonParser.VGLOBAL: {
			e.info = this.codeABx(OP_GETGLOBAL, 0, e.info);
			e.k = MoonParser.VRELOCABLE;
			break;
		}
		case MoonParser.VINDEXED: {
			this.freereg(e.aux);
			this.freereg(e.info);
			e.info = this
		...
		case MoonParser.VVARARG:
		case MoonParser.VCALL: {
			this.setoneret(e);
			break;
		}
		default:
	...
	void discharge2reg(ExpDesc e, int reg) {
		...
		switch (e.k) {
		case MoonParser.VNIL: {
			this.nil(reg, 1);
			break;
		}
		case MoonParser.VFALSE:
		case MoonParser.VTRUE: {
			this.codeABC(OP_LOADBOOL, reg, (e.k == MoonParser.VTRUE ? 1 : 0),
					0);
			break;
		}
		case MoonParser.VK: {
		...
		case MoonParser.VKNUM: {
			this.codeABx(OP_LOADK, reg, this.numberK(e.nval()));
			break;
		}
		case MoonParser.VRELOCABLE: {
			InstructionPtr pc = this.getcodePtr(e);
			SETARG_A(pc, reg);
			break;
		}
		case MoonParser.VNONRELOC: {
			if (reg != e.info)
				this.codeABC(OP_MOVE, reg, e.info, 0);
			break;
		}
		default: {
			_assert (e.k == MoonParser.VVOID || e.k == MoonParser.VJMP);
	...
	void exp2reg(ExpDesc e, int reg) {
		...
		if (e.k == MoonParser.VJMP)
			e.t = this.concat(e.t, e.info); /* put this jump in `t' list */
		if (e.hasjumps()) {
			int _final; /* position after whole expression */
			int p_f = MoonParser.NO_JUMP; /* position of an eventual LOAD false */
			int p_t = MoonParser.NO_JUMP; /* position of an eventual LOAD true */
			if (this.need_value(e.t) || this.need_value(e.f)) {
				int fj = (e.k == MoonParser.VJMP) ? MoonParser.NO_JUMP : this
						.jump();
	...
	int exp2RK(ExpDesc e) {
		...
		case MoonParser.VNIL: {
			if (this.nk <= MAXINDEXRK) { /* constant fit in RK operand? */
				e.info = (e.k == MoonParser.VNIL) ? this.nilK()
						: (e.k == MoonParser.VKNUM) ? this.numberK(e.nval())
								: this.boolK((e.k == MoonParser.VTRUE));
				e.k = MoonParser.VK;
				return RKASK(e.info);
			} else
				break;
	...
	void goiftrue(ExpDesc e) {
		...
		case MoonParser.VKNUM:
		case MoonParser.VTRUE: {
			pc = MoonParser.NO_JUMP; /* always true; do nothing */
			break;
		}
		case MoonParser.VFALSE: {
	...
	void goiffalse(ExpDesc e) {
		...
		case MoonParser.VNIL:
		case MoonParser.VFALSE: {
			pc = MoonParser.NO_JUMP; /* always false; do nothing */
			break;
		}
		case MoonParser.VTRUE: {
			pc = this.jump(); /* always jump */
	...
	void codenot(ExpDesc e) {
		...
		case MoonParser.VNIL:
		case MoonParser.VFALSE: {
			e.k = MoonParser.VTRUE;
			break;
		}
		case MoonParser.VK:
		case MoonParser.VKNUM:
		case MoonParser.VTRUE: {
			e.k = MoonParser.VFALSE;
			break;
		}
		case MoonParser.VJMP: {
			this.invertjump(e);
	...
	void codecomp(int /* OpCode */op, int cond, ExpDesc e1, ExpDesc e2) {
		...
		e1.k = MoonParser.VJMP;
	}

	void prefix(int /* UnOpr */op, ExpDesc e) {
		ExpDesc e2 = new ExpDesc();
		e2.init(MoonParser.VKNUM, 0);
		switch (op) {
		case MoonParser.OPR_MINUS: {
			if (e.k == MoonParser.VK)
				this.exp2anyreg(e); /* cannot operate on non-numeric constants */
			this.codearith(OP_UNM, e, e2);
			break;
		}
		case MoonParser.OPR_NOT:
			this.codenot(e);
			break;
		case MoonParser.OPR_LEN: {
	...
	void infix(int /* BinOpr */op, ExpDesc v) {
		switch (op) {
		case MoonParser.OPR_AND: {
			this.goiftrue(v);
			break;
		}
		case MoonParser.OPR_OR: {
			this.goiffalse(v);
			break;
		}
		case MoonParser.OPR_CONCAT: {
			this.exp2nextreg(v); /* operand must be on the `stack' */
			break;
		}
		case MoonParser.OPR_ADD:
		case MoonParser.OPR_SUB:
		case MoonParser.OPR_MUL:
		case MoonParser.OPR_DIV:
		case MoonParser.OPR_MOD:
		case MoonParser.OPR_POW: {
			if (!v.isnumeral())
				this.exp2RK(v);
			break;
		}
	...
	void posfix(int op, ExpDesc e1, ExpDesc e2) {
		switch (op) {
		case MoonParser.OPR_AND: {
			_assert (e1.t == MoonParser.NO_JUMP); /* list must be closed */
		...
		case MoonParser.OPR_OR: {
			_assert (e1.f == MoonParser.NO_JUMP); /* list must be closed */
			this.dischargevars(e2);
	...
	void setlist(int base, int nelems, int tostore) {
		...
		int b = (tostore == MOON_MULTRET) ? 0 : tostore;
		_assert (tostore != 0);
		if (c <= MAXARG_C)
...
//			throw new KahluaException("compiler assert failed");
	...
	public static final int MOON_MULTRET = -1;
